...
func mintCreds(config *Config) *cobra.Command {
...
Examples:
  ...
  voidkey mint --keys MINIO_CREDENTIALS --duration 1800`,
		RunE: func(cobraCmd *cobra.Command, args []string) error {
			...
			return mintCredentialsWithFlags(client, config, cobraCmd, localOidcToken, localOutputFormat, localIdpName, localKeys, localDuration, localAll)
		},
	}

	// Flags for the mint command
	cmd.Flags().StringVar(&localOidcToken, "token", "", "OIDC token for authentication (overrides config and env vars)")
	cmd.Flags().StringVarP(&localOutputFormat, "output", "o", "env", "Output format (env|json)")
	cmd.Flags().StringVar(&localIdpName, "idp", config.IdpName, "IdP provider name to use (overrides config)")
...
func mintCredentialsWithFlags(client *VoidkeyClient, config *Config, cmd *cobra.Command, token, format, idpName string, keys []string, duration int, all bool) error {
	// Check for token from config and environment variables if not provided via flag
	if token == "" {
		var envVarName string
		token, envVarName = config.GetTokenWithSource()
		if token != "" {
			_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "🔍 Using %s environment variable\n", envVarName)
		}
	}
	...
	if token == "" {
		return fmt.Errorf("OIDC token is required. Provide via:\n"+
			...
			"  Environment var: export %s=\"your.jwt.token\"\n"+
			"  Configuration: voidkey config set token-env GITHUB_TOKEN\n\n"+
			...
			"  - Hello World: Use --idp hello-world for testing (no token required)", config.TokenEnv)
	}

	// Show IdP selection info
	if idpName != "" {
		if idpName == config.IdpName {
			_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "🔍 Using IdP provider: %s (from config)\n", idpName)
		} else {
			_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "🔍 Using IdP provider: %s (override)\n", idpName)
		}
	} else {
